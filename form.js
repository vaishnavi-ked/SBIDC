...
window.clickSubmit = function(event) {
    ...
    // Get form values
    const dcName = $("input[placeholder='DC Name']").val();
    console.log("dcName:", dcName);
    const appName = $("input[placeholder='Application Name']").val();
    console.log("appName:", appName);
    const aoName = $("input[placeholder='Ao Name']").val();
    console.log("aoName:", aoName);
    const serverHall = $("#server-hall").val();
    console.log("serverHall:", serverHall);
    const row = $("#row-dropdown").val();
    console.log("row:", row);
    const rack = $("#rack-dropdown").val();
    console.log("rack:", rack);
    const powerCapacity = $("input[placeholder='Power Capacity']").val();
    console.log("powerCapacity:", powerCapacity);
    const allotedPower = $("input[placeholder='Alloted Power']").val();
    console.log("allotedPower:", allotedPower);
    const actualPower = $("input[placeholder='Actual Power']").val();
    console.log("actualPower:", actualPower);
    const availablePower = $("input[placeholder='Available Power']").val();
    console.log("availablePower:", availablePower);

    // Validate required fields
    if (!appName || !aoName || !serverHall || !row || !rack) {
        console.log("Validation failed: Missing required fields");
        dialogs.showOk({ content: "Please fill all required fields" });
        return false;
    }
    console.log("Validation passed");

    // Find the selected rack to get its full path
    const rackData = DataService.getRacks();
    console.log("rackData:", rackData);
    const selectedHall = rackData.find(h => h.serverHall === serverHall);
    ...
    if (!selectedRack) {
        console.log("Selected rack not found");
        dialogs.showOk({ content: "Selected rack not found" });
        return false;
    }

    // Show confirmation dialog
    dialogs.showOk({
        content: `Are you sure you want to update these points?`,
        okCallback: function() {
            console.log("User confirmed update");

            // Update string points
            console.log("Updating applicationName...");
            updateStringPoint(`${selectedRack.fullPath}/applicationName`, appName);
            console.log("Updating AoName...");
            updateStringPoint(`${selectedRack.fullPath}/AoName`, aoName);

            // Update numeric points with override
            console.log("Updating powerCapacity...");
            updateNumericPointWithOverride(`${selectedRack.fullPath}/powerCapacity`, powerCapacity);
            console.log("Updating allotedPower...");
            updateNumericPointWithOverride(`${selectedRack.fullPath}/allotedPower`, allotedPower);
            console.log("Updating actualPower...");
            updateNumericPointWithOverride(`${selectedRack.fullPath}/actualPower`, actualPower);
            console.log("Updating availablePower...");
            updateNumericPointWithOverride(`${selectedRack.fullPath}/availablePower`, availablePower);

            console.log("Data successfully sent to Niagara");
        }
    });
...
async function updateStringPoint(ordPath, value) {
    console.log(`Attempting to update string point at ${ordPath} with value ${value}`);
    try {
        const point = await baja.Ord.make(ordPath).get();
        console.log("Point fetched:", point);
        if (!point) {
            console.error(`Point not found at ${ordPath}`);
            dialogs.showOk({ content: `Point not found at ${ordPath}` });
            return;
        }

        // Create a StringWritable object
        const stringWritable = baja.$("control:StringWritable", {
            value: value
        });
        console.log("Created StringWritable:", stringWritable);

        // Write the value
        await point.invoke({
            slot: 'write',
            value: stringWritable
        });
        console.log(`Successfully updated string point ${ordPath} to ${value}`);
    } catch (err) {
        console.error(`Error updating string point ${ordPath}:`, err);
        dialogs.showOk({ content: `Failed to update ${ordPath}: ${err.message}` });
    }
}

// Enhanced numeric point update with override
async function updateNumericPointWithOverride(ordPath, value) {
    console.log(`Attempting to override numeric point at ${ordPath} with value ${value}`);
    try {
        const numValue = parseFloat(value);
        console.log("Parsed numeric value:", numValue);
        if (isNaN(numValue)) {
            console.error(`Invalid number value: ${value}`);
            dialogs.showOk({ content: `Invalid number value: ${value}` });
            return;
        }
        ...
        // Create a NumericOverride object
        const overrideVal = baja.$("control:NumericOverride", {
            value: numValue,
            duration: baja.RelTime.make({ seconds: 0 }) // 0 = permanent override
        });
        console.log("Created NumericOverride:", overrideVal);
        ...
        console.log(`Successfully overridden numeric point ${ordPath} to ${numValue}`);
    } catch (err) {
        console.error(`Error overriding numeric point ${ordPath}:`, err);
        dialogs.showOk({ content: `Failed to override ${ordPath}: ${err.message}` });
    }
}
  ...
  // });
   $(document).ready(function() {
        // Initialize UI and load data as before
        UIManager.initialize();
        ...
        // Update form submit button to use window.clickSubmit
